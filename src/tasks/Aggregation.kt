...
fun List<User>.aggregate(): List<User> =
    this.groupingBy { user -> user.login }
        .aggregate { login: String, accumulator: User?, user: User, _: Boolean ->
            User(login, contributions = (accumulator?.contributions ?: 0) + user.contributions)
        }.values
